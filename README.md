...
3. **属性アイコンを選択**
   - 以下のカテゴリから選択：
     ...
     - **自由アイコン**: 上記カテゴリに該当しない独自アイコンを追加可能
...
#### 自由アイコン（Freeform）
既存のカテゴリに該当しないアイコンをバッジの末尾に追加できます。
1. 「自由アイコン」セクションで「画像を追加」または「テキストを追加」をクリック
2. **画像の場合**: 画像ファイルを選択
3. **テキストの場合**: アイコン内に表示するテキストを入力（複数行対応）
4. 帯に表示するテキスト（4文字まで）を入力
5. 「追加」ボタンで確定
6. 追加後も✎ボタンで再編集可能、×ボタンで削除
